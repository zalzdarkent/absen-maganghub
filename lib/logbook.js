...
import { readEnvFile, readSettingsFile } from './settings.js';
...
function normalizeModel(raw) {
    ...
    // Model 1.5-flash tidak didukung di API v1beta free tier — arahkan ke DEFAULT_MODEL (gemini-3.6-flash)
...
async function generateWithOpenAICompatible(prompt, options = {}) {
    const rawUrl = options.baseURL || options.localLlmUrl || 'http://192.168.13.155:8080/v1';
    const baseURL = String(rawUrl).replace(/\/+$/, '');
    const model = options.model || options.localLlmModel || 'gpt-oss-20b';
    const apiKey = options.apiKey || options.localLlmApiKey || 'none';
    ...
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(`Local LLM (${model}) HTTP error ${res.status}: ${errText.slice(0, 200)}`);
        }

        const data = await res.json();
        const raw = data?.choices?.[0]?.message?.content || '';
        if (!raw) {
            throw new Error('Local LLM mengembalikan respon kosong.');
        }

        const parsed = extractJsonObject(raw);
        for (const k of ['aktivitas', 'pembelajaran', 'kendala']) {
            if (typeof parsed[k] !== 'string' || parsed[k].trim().length < 10) {
                throw new Error(`Field "${k}" dari Local LLM tidak valid / terlalu pendek. Coba generate ulang.`);
            }
        ...
        if (err.name === 'AbortError' || String(err.message).toLowerCase().includes('abort')) {
            throw new Error(`Local LLM timeout (>60 detik) saat menghubungi ${baseURL}. Pastikan server local LLM aktif.`);
        }
        if (err.code === 'ECONNREFUSED' || err.message?.includes('ECONNREFUSED') || err.message?.includes('fetch failed')) {
            throw new Error(`Tidak dapat terhubung ke Local LLM di ${baseURL}. Pastikan PC/server lokal (192.168.13.155) menyala dan port 8080 terbuka.`);
        }
        throw err;
    }
}

async function generateRecapRaw(prompt, options = {}) {
    const env = { ...readEnvFile(), ...process.env };
    let settings = {};
    try { settings = await readSettingsFile(); } catch {}
    const provider = options.provider || settings.llmProvider || env.LLM_PROVIDER || 'inoacGPT';

    if (provider === 'inoacGPT' || provider === 'local' || provider === 'openai') {
        const rawUrl = options.baseURL || options.localLlmUrl || settings.localLlmUrl || env.LOCAL_LLM_URL || 'http://192.168.13.155:8080/v1';
        const baseURL = String(rawUrl).replace(/\/+$/, '');
        const model = options.model || options.localLlmModel || settings.localLlmModel || env.LOCAL_LLM_MODEL || 'gpt-oss-20b';
        const apiKey = options.apiKey || options.localLlmApiKey || settings.localLlmApiKey || env.LOCAL_LLM_API_KEY || 'none';
        const TIMEOUT_MS = 60000;
        ...
        try {
            ...
            const res = await fetch(endpoint, {
                ...
                body: JSON.stringify({
                    model,
                    messages: [
                        { role: 'system', content: 'Kamu adalah asisten penulisan laporan magang untuk MagangHub Kemnaker yang menghasilkan respon JSON valid saja.' },
            ...
            clearTimeout(timeoutId);
            if (err.code === 'ECONNREFUSED' || err.message?.includes('ECONNREFUSED') || err.message?.includes('fetch failed')) {
                throw new Error(`Tidak dapat terhubung ke Local LLM di ${baseURL}. Pastikan server lokal aktif.`);
            }
            throw err;
        }
    }
    ...
    if (!apiKey) throw new Error('GEMINI_API_KEY belum diatur. Isi di Pengaturan atau switch ke Local LLM (inoacGPT).');
    const primaryModel = normalizeModel(options.geminiModel || env.GEMINI_MODEL);
...
function extractJsonObject(rawText) {
    ...
    throw new Error('Model AI mengirim format jawaban yang tidak valid. Silakan coba generate ulang.');
}

async function generateWithGemini(gitLogs, diffSection = '', options = {}) {
    return generateFromPrompt(buildPrompt(gitLogs, diffSection), options);
}

async function generateManualWithGemini(description, options = {}) {
    return generateFromPrompt(buildManualPrompt(description), options);
}

async function generateCombinedWithGemini(gitLogs, manualNotes, diffSection = '', options = {}) {
    return generateFromPrompt(buildCombinedPrompt(gitLogs, manualNotes, diffSection), options);
}
...
async function generateFromPrompt(prompt, options = {}) {
    ...
    if (provider === 'inoacGPT' || provider === 'local' || provider === 'openai') {
        const localLlmUrl = options.localLlmUrl || settings.localLlmUrl || env.LOCAL_LLM_URL || 'http://192.168.13.155:8080/v1';
        const localLlmModel = options.localLlmModel || settings.localLlmModel || env.LOCAL_LLM_MODEL || 'gpt-oss-20b';
        const localLlmApiKey = options.localLlmApiKey || settings.localLlmApiKey || env.LOCAL_LLM_API_KEY || 'none';
        return generateWithOpenAICompatible(prompt, { localLlmUrl, localLlmModel, localLlmApiKey });
    }
    ...
    for (let attempt = 0; attempt < modelsToTry.length; attempt++) {
        ...
        try {
            ...
            if (msg === 'TIMEOUT') {
                throw new Error(`Gemini lama merespons (>${Math.round(TIMEOUT_MS/1000)} detik). Sudah coba ${attempt+1}x. Coba lagi atau switch ke Local LLM (inoacGPT).`);
            }
            const detail = msg;
            ...
            if (detail.includes('429') || detail.toLowerCase().includes('quota') || detail.toLowerCase().includes('rate')) {
                throw new Error('Kuota Gemini habis / rate limit. Tunggu 1-2 menit lalu coba lagi, atau switch ke Local LLM (inoacGPT).');
            }
            if (detail.toLowerCase().includes('model') && detail.toLowerCase().includes('not found')) {
                throw new Error(`Model Gemini "${model}" tidak ditemukan. Coba ganti ke gemini-3.6-flash di Pengaturan.`);
            }
...
async function testLlmConnection(options = {}) {
    ...
    if (provider === 'inoacGPT' || provider === 'local') {
        const rawUrl = options.localLlmUrl || settings.localLlmUrl || env.LOCAL_LLM_URL || 'http://192.168.13.155:8080/v1';
        const baseURL = String(rawUrl).replace(/\/+$/, '');
        const model = options.localLlmModel || settings.localLlmModel || env.LOCAL_LLM_MODEL || 'gpt-oss-20b';
        const apiKey = options.apiKey || options.localLlmApiKey || settings.localLlmApiKey || env.LOCAL_LLM_API_KEY || 'none';
        ...
        const res = await fetch(endpoint, {
            ...
            body: JSON.stringify({
                model,
                messages: [{ role: 'user', content: 'Halo! Balas: {"status":"ok"}' }],
                max_tokens: 50,
            }),
        });
        ...
        return { ok: true, provider: 'inoacGPT', model, latencyMs, message: `Terhubung ke Local LLM (${model}) — ${latencyMs}ms` };
    } else {
        const apiKey = options.apiKey || env.GEMINI_API_KEY;
        if (!apiKey) throw new Error('GEMINI_API_KEY belum diisi.');
        const model = normalizeModel(options.geminiModel || env.GEMINI_MODEL);
        const ai = new GoogleGenAI({ apiKey });
        await ai.models.generateContent({
            model,
            contents: 'Halo, balas singkat JSON: {"status":"ok"}',
        });
        const latencyMs = Date.now() - t0;
        return { ok: true, provider: 'gemini', model, latencyMs, message: `Terhubung ke Google Gemini (${model}) — ${latencyMs}ms` };
    }
}
