...
const LOGBOOK_FILE_PATH = path.join(process.cwd(), 'logbook.json');
...
function readLogbookFile() {
    if (!fs.existsSync(LOGBOOK_FILE_PATH)) return [];
    try {
        const entries = JSON.parse(fs.readFileSync(LOGBOOK_FILE_PATH, 'utf-8'));
...
function writeLogbookFile(entries) {
    fs.writeFileSync(LOGBOOK_FILE_PATH, JSON.stringify(entries, null, 2) + '\n', 'utf-8');
}
