...
function buildDraftReadyPayload(commitCount = 0) {
    const countText = commitCount > 0 ? ` dari ${commitCount} commit` : '';
    return {
        title: 'Draft logbook siap',
        body: `Draft hari ini sudah jadi${countText}. Klik untuk cek dan lengkapi.`,
        tag: `draft-ready-${todayKeyWIB()}`,
        url: '/?draft=ready',
    };
}

function buildNoCommitPayload() {
    return {
        title: 'Belum ada commit hari ini',
        body: 'Hari ini kamu belum push commit ke GitHub — yuk segera buat catatan manual biar logbook tetap keisi.',
        tag: `no-commit-${todayKeyWIB()}`,
        url: '/?action=manual',
    };
}
...
async function sendReminderToAll(payload = buildReminderPayload()) {
    ...
    if (subs.length === 0) {
        console.warn('[push] sendReminderToAll: tidak ada subscriber terdaftar. Pastikan sudah subscribe push di Settings.');
    ...
    await Promise.all(subs.map(async (sub) => {
        try {
            ...
            sent += 1;
            console.log(`[push] OK endpoint=...${sub.endpoint.slice(-20)}`);
        } catch (e) {
            const status = e && (e.statusCode || e.status);
            const msg = e && e.message ? e.message : String(e);
            console.warn(`[push] GAGAL endpoint=...${sub.endpoint.slice(-20)} status=${status} msg=${msg}`);
            if (status === 404 || status === 410) {
                console.warn(`[push] Subscription expired/invalid (${status}), akan dihapus. Perlu re-subscribe dari browser.`);
                dead.push(sub.endpoint);
                errors.push({ endpoint: sub.endpoint.slice(-20), status, reason: 'expired' });
            } else {
                failed += 1;
                errors.push({ endpoint: sub.endpoint.slice(-20), status, reason: msg });
            }
        }
    }));
    let removed = 0;
    for (const ep of dead) {
        try {
            ...
            removed += 1;
            console.warn(`[push] Subscription dihapus (expired): ...${ep.slice(-20)}`);
        } catch {
            // abaikan
        }
    }
    if (removed > 0) {
        console.warn(`[push] ${removed} subscription dihapus karena expired. Buka Settings dan aktifkan Web Push lagi!`);
    }
    return { sent, failed, removed, total: subs.length, errors };
}

async function getSubscriptionDebugInfo() {
    const subs = await listSubscriptions();
    const lastSentMap = await getLastSentMap();
    const { publicKey, subject } = getVapidKeys();
    return {
        configured: isPushConfigured(),
        vapidSubject: subject,
        vapidPublicKeyPrefix: publicKey ? publicKey.slice(0, 20) + '...' : null,
        subscriptionCount: subs.length,
        subscriptions: subs.map(s => ({
            endpointSuffix: '...' + s.endpoint.slice(-30),
            endpointProvider: s.endpoint.includes('fcm.googleapis.com') ? 'FCM/Chrome'
                : s.endpoint.includes('mozilla') ? 'Mozilla/Firefox'
                : s.endpoint.includes('windows') ? 'WNS/Edge'
                : 'Unknown',
            createdAt: s.createdAt || null,
            daysAgo: s.createdAt ? Math.floor((Date.now() - new Date(s.createdAt).getTime()) / 86400000) : null,
            expirationTime: s.expirationTime || null,
        })),
        lastSentMap,
        todayKey: todayKeyWIB(),
        hasSentToday: Boolean(lastSentMap[todayKeyWIB()]),
    };
}
