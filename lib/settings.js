...
const KNOWN_ENV_KEYS = ['GEMINI_API_KEY', 'GITHUB_TOKEN', 'GEMINI_MODEL', 'LLM_PROVIDER', 'LOCAL_LLM_URL', 'LOCAL_LLM_MODEL', 'LOCAL_LLM_API_KEY'];
...
async function getSettingsForDisplay() {
    ...
    const llmProvider = env.LLM_PROVIDER || settings.llmProvider || 'local';
    const localLlmUrl = env.LOCAL_LLM_URL || settings.localLlmUrl || 'http://192.168.13.155:3000';
    const localLlmModel = env.LOCAL_LLM_MODEL || settings.localLlmModel || 'gpt-oss-20b';
    const localLlmApiKey = env.LOCAL_LLM_API_KEY || settings.localLlmApiKey || '';
    const hasSettingsRepo = settings.repoPath !== undefined && String(settings.repoPath).trim() !== '';
    ...
    return {
        ...
        localLlmUrl,
        localLlmModel,
        localLlmApiKey: localLlmApiKey ? `${localLlmApiKey.slice(0, 4)}${'•'.repeat(Math.max(localLlmApiKey.length - 8, 4))}${localLlmApiKey.slice(-4)}` : '',
...
async function saveSettings({ apiKey, repoPath, geminiModel, githubToken, repositories, activeRepoId, defaultRepoIds, llmProvider, localLlmUrl, localLlmModel, localLlmApiKey } = {}) {
    const toSaveInSettings = {};
    if (llmProvider !== undefined && String(llmProvider).trim() !== '') {
        toSaveInSettings.llmProvider = String(llmProvider).trim();
    }
    if (localLlmUrl !== undefined && String(localLlmUrl).trim() !== '') {
        toSaveInSettings.localLlmUrl = String(localLlmUrl).trim();
    }
    if (localLlmModel !== undefined && String(localLlmModel).trim() !== '') {
        toSaveInSettings.localLlmModel = String(localLlmModel).trim();
    }
    if (localLlmApiKey !== undefined) {
        toSaveInSettings.localLlmApiKey = String(localLlmApiKey).trim();
    }
    if (repositories !== undefined) {
        const sanitized = sanitizeRepositories(repositories);
        toSaveInSettings.repositories = sanitized;
        if (activeRepoId !== undefined) {
            const id = String(activeRepoId || '').trim();
            const ids = new Set(sanitized.map(r=>r.id));
            if (!id || !ids.has(id)) {
                if (sanitized.length) toSaveInSettings.activeRepoId = sanitized[0].id;
            } else {
                toSaveInSettings.activeRepoId = id;
            }
        }
        if (defaultRepoIds !== undefined) {
            ...
            toSaveInSettings.defaultRepoIds = filtered.length? filtered : sanitized.slice(0,1).map(r=>r.id);
        }
    }
    if (repoPath !== undefined) {
        const trimmed = String(repoPath).trim();
        if (!trimmed.includes('***')) {
            toSaveInSettings.repoPath = trimmed;
            ...
            if (!current.repositories || current.repositories.length===0) {
                ...
                if (repos.length) {
                    toSaveInSettings.repositories = repos;
                    toSaveInSettings.activeRepoId = repos[0].id;
                    toSaveInSettings.defaultRepoIds = [repos[0].id];
                }
            }
        }
    }
    if (Object.keys(toSaveInSettings).length > 0) {
        await writeSettingsFile(toSaveInSettings);
    }
    const toWriteEnv = {};
    ...
    if (llmProvider !== undefined && String(llmProvider).trim() !== '') {
        toWriteEnv.LLM_PROVIDER = String(llmProvider).trim();
    }
    if (localLlmUrl !== undefined && String(localLlmUrl).trim() !== '') {
        toWriteEnv.LOCAL_LLM_URL = String(localLlmUrl).trim();
    }
    if (localLlmModel !== undefined && String(localLlmModel).trim() !== '') {
        toWriteEnv.LOCAL_LLM_MODEL = String(localLlmModel).trim();
    }
    if (localLlmApiKey !== undefined) {
        toWriteEnv.LOCAL_LLM_API_KEY = String(localLlmApiKey).trim();
    }
