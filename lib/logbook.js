...
async function generateWithOpenAICompatible(prompt, options = {}) {
    const rawUrl = options.baseURL || options.localLlmUrl || 'http://192.168.13.155:3000';
    ...
    try {
        ...
        if (err.code === 'ECONNREFUSED' || err.message?.includes('ECONNREFUSED') || err.message?.includes('fetch failed')) {
            throw new Error(`Tidak dapat terhubung ke Local LLM di ${baseURL}. Pastikan server lokal menyala dan port terbuka.`);
        }
        throw err;
    }
}
...
async function generateRecapRaw(prompt, options = {}) {
    ...
    const provider = options.provider || settings.llmProvider || env.LLM_PROVIDER || 'local';

    if (provider === 'local' || provider === 'openai') {
        const rawUrl = options.baseURL || options.localLlmUrl || settings.localLlmUrl || env.LOCAL_LLM_URL || 'http://192.168.13.155:3000';
        const baseURL = String(rawUrl).replace(/\/+$/, '');
    ...
    if (!apiKey) throw new Error('GEMINI_API_KEY belum diatur. Isi di Pengaturan atau switch ke Local LLM.');
...
async function generateFromPrompt(prompt, options = {}) {
    ...
    if (provider === 'local' || provider === 'openai') {
        const localLlmUrl = options.localLlmUrl || settings.localLlmUrl || env.LOCAL_LLM_URL || 'http://192.168.13.155:3000';
    ...
    for (let attempt = 0; attempt < modelsToTry.length; attempt++) {
        ...
        try {
            ...
            if (msg === 'TIMEOUT') {
                throw new Error(`Gemini lama merespons (>${Math.round(TIMEOUT_MS/1000)} detik). Sudah coba ${attempt+1}x. Coba lagi atau switch ke Local LLM.`);
            }
            const detail = msg;
            ...
            if (detail.includes('429') || detail.toLowerCase().includes('quota') || detail.toLowerCase().includes('rate')) {
                throw new Error('Kuota Gemini habis / rate limit. Tunggu 1-2 menit lalu coba lagi, atau switch ke Local LLM.');
            }
...
async function testLlmConnection(options = {}) {
    ...
    if (provider === 'local') {
        const rawUrl = options.localLlmUrl || settings.localLlmUrl || env.LOCAL_LLM_URL || 'http://192.168.13.155:3000';
        const baseURL = String(rawUrl).replace(/\/+$/, '');
        ...
        return { ok: true, provider: 'local', model, latencyMs, message: `Terhubung ke Local LLM (${model}) — ${latencyMs}ms` };
    } else {
        const apiKey = options.apiKey || env.GEMINI_API_KEY;
